...
Ext.onReady(function(){
	...
	var loginForm = new Ext.form.FormPanel({
		...
		items: [{
			 ...
			 listeners : {
                specialkey : function(field, e) {//添加回车事件
                    if (e.getKey() == Ext.EventObject.ENTER) {
                        loginForm.getForm().getEl().dom.action="system/login.do";
			...
			items:[{
			xtype: 'panel',
			html: '<img src="validate.jsp" title="点击刷新" style="cursor:hand" id="validate" onclick="refreshCode()"/>'
			}]
		},{
			layout: 'table',
			layoutConfig:{
...
function refreshCode(){
	Ext.getDom('validate').src="validate.jsp?code="+Math.random();
}
