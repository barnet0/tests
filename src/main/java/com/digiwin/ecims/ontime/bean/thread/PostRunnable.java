...
public class PostRunnable<T> implements Runnable {
    private static final Logger logger = LoggerFactory.getLogger(PostRunnable.class);
    ...
    @Override public void run() {
        ...
        for (PostDataObject<T> postDataObject : postDataObjects) {
            ...
            result = this.doPost(postDataObject);
            ...
            try {
                ...
                if (resultMaps.get("result").equals("fail")) {
                    ...
                    // 将结果添加到日志文件备查
                    logger.error("第{}组推送失败,资料类型为{},响应为 {}", postDataObjectGroupIdx, clazz, result);
                }
                ...
                logger.info("第{}组推送结束,资料类型为{},耗时{}毫秒,响应为 {}", postDataObjectGroupIdx, clazz,
                    this.postResDate.getTime() - this.postDate.getTime(), result);
            }
        }
        // add by mowj 20150827
