...
@Service("jingdongAfsUpdateCheckServiceImpl")
public class JingdongAfsUpdateCheckServiceImpl implements OnTimeTaskCheckBusiJob {

  private static final Logger logger =
      LoggerFactory.getLogger(JingdongAfsUpdateCheckServiceImpl.class);
  ...
  public boolean timeOutExecuteCheck(String... args) throws Exception {
     ...
     * ex: taobaoTbTradeUpdateCheck#0#Hour#1，表示淘宝普通服务单，检验前一小时至当前时间的服务单
     * 
...
//  每次執行, 往回取幾個小時的資料
    ...
    if (aomsShopList != null && aomsShopList.size() != 0) {
      logger.info("开始--京东服务单补单---排程类型{}", scheduleType);
...
//      add by mowj 20151118 调整为取当前补单排程启动的时间
        ...
        baseCheckUpdateService.executeRefundUpdateCheck(
            jingdongApiSyncReturnsService, scheduleType, aomsshopT, checkDate[0], checkDate[1]);

      }
      // 更新 lastRunTime add by xaiver on 20150906
      baseCheckUpdateService.updateLastRunTime(lastRunTime, scheduleType);
      
      logger.info("结束--京东服务单补单---排程类型{},上次运行时间{}", scheduleType, DateTimeTool.format(lastRunTime));
      return true;
    } else {
      logger.info("京东服务单补单失败---没有找到相应的店铺,请确认排程类型{}设定是否正确", scheduleType);
