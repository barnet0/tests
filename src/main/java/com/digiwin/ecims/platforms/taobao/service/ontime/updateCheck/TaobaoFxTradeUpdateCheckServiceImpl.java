package com.digiwin.ecims.platforms.taobao.service.ontime.updateCheck;
...
@Service("taobaoFxTradeUpdateCheckServiceImpl")
public class TaobaoFxTradeUpdateCheckServiceImpl implements OnTimeTaskCheckBusiJob {

  private static final Logger logger =
      LoggerFactory.getLogger(TaobaoFxTradeUpdateCheckServiceImpl.class);
...
//  每次執行, 往回取幾個小時的資料
    ...
    if (aomsShopList != null && aomsShopList.size() != 0) {
      logger.info("开始--淘宝分销订单补单---排程类型{}", scheduleType);
...
//      add by mowj 20151118 调整为取当前补单排程启动的时间
        ...
        baseCheckUpdateService.executeOrderUpdateCheck(
            taobaoFxApiSyncOrdersService, scheduleType, aomsshopT, checkDate[0], checkDate[1]);

      }
      // 更新 lastRunTime add by xaiver on 20150906
      baseCheckUpdateService.updateLastRunTime(lastRunTime, scheduleType);
      
      logger.info("结束--淘宝分销订单补单---排程类型{},上次运行时间{}", scheduleType, DateTimeTool.format(lastRunTime));
      return true;
    } else {
      logger.info("淘宝分销订单补单失败---没有找到相应的店铺,请确认排程类型{}设定是否正确", scheduleType);
