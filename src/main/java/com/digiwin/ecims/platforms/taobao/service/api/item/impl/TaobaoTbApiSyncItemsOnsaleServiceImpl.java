...
@Service
public class TaobaoTbApiSyncItemsOnsaleServiceImpl implements TaobaoTbApiSyncItemsOnsaleService {
  ...
  public void syncData(TaskScheduleConfig taskScheduleConfig, AomsshopT aomsshopT)
      throws Exception {
    syncTaobaoItemsOnsale(taskScheduleConfig, aomsshopT);
  }

  @Override
  public Boolean syncItems(TaskScheduleConfig taskScheduleConfig, AomsshopT aomsshopT)
      throws Exception {
    return syncTaobaoItemsOnsale(taskScheduleConfig, aomsshopT);
  }

  @Override
  public Boolean syncTaobaoItemsOnsale(TaskScheduleConfig taskScheduleConfig, AomsshopT aomsshopT)
    ...
    ItemsOnsaleGetResponse itemsOnsaleGetResponse = 
        taobaoApiService.taobaoItemsOnsaleGet(
    ...
    long totalSize = itemsOnsaleGetResponse.getTotalResults();
 ...
 // 整理搜尋區間，直到區間數目<=最大讀取數目
    ...
    while (totalSize > taskScheduleConfig.getMaxReadRow()) {
      ...
      itemsOnsaleGetResponse = 
          taobaoApiService.taobaoItemsOnsaleGet(
              ...
              DateTimeTool.parse(sDate), DateTimeTool.parse(eDate), 
              TaobaoCommonTool.MIN_PAGE_NO, (long)pageSize);
      totalSize = itemsOnsaleGetResponse.getTotalResults() == null ? 
          0L : itemsOnsaleGetResponse.getTotalResults();
    }
    ...
    for (long i = pageNum; i > 0; i--) {
      ...
      loginfoOperateService.newTransaction4SaveSource(sDate, eDate, TaobaoCommonTool.STORE_TYPE,
          "[" + UseTimeEnum.UPDATE_TIME + "]|taobao.items.onsale.get 获取当前会话用户出售中的商品列表",
    ...
    if (itemIds.size() > 0) {
      ...
      for (int i = 0; i < itemIds.size(); i++) {
        ...
        if (((i + 1) % 20 != 0) && (i + 1) < itemIds.size()) {
          ...
          ItemsSellerListGetResponse itemsListResponse = 
              taobaoApiService.taobaoItemsSellerListGet(
