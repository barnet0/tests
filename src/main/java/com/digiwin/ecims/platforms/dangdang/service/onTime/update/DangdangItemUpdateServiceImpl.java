...
@Service("dangdangItemUpdateServiceImpl")
public class DangdangItemUpdateServiceImpl extends UpdateTask implements OnTimeTaskBusiJob {

  private static final Logger logger = LoggerFactory.getLogger(DangdangItemUpdateServiceImpl.class);
  ...
  public boolean timeOutExecute(String... args) throws Exception {
    ...
    logger.info("定时任务开始--DangdangItemUpdateServiceImpl--获取当当商品数据");
    TaskScheduleConfig taskScheduleConfig = null;
    String scheduleType = null;
    try {
      /**
       * 通过分销类型获得分销商的铺货信息 0:淘宝,1:京东,2:一号店,3:工行,4:苏宁,5:当当,6:拍拍,9:其他,A:淘宝分销,B:QQ网购,C:卓越亚马逊
       */
      List<AomsshopT> aomsShopList =
          aomsShopService.getStoreByStoreType(DangdangCommonTool.STORE_TYPE);
      if (aomsShopList != null && aomsShopList.size() != 0) {
        for (int i = 0; i < aomsShopList.size(); i++) {
           ...
           * 此处当当的格式为：DangdangItemUpdate#DZ0004#5
           */
          scheduleType =
              "DangdangItemUpdate#" + aomsshopT.getAomsshop001() + "#" + aomsshopT.getAomsshop003();
          /*
           * 通过“DangdangItemUpdate”当当的定时任务计划类型， 获取定时任务的schedule配置， 此配置内存放了该批次的定时任务的最后更新时间，每笔最大更新条数
           */
          taskScheduleConfig = taskService.getTaskScheduleConfigInfo(scheduleType);
          taskService.newTransaction4SaveLastRunTime(scheduleType, null); // 記錄執行時間, add by xavier
                                                                          // on 20150830
          ...
          dangdangAPIService.syncGoodsData(taskScheduleConfig, aomsshopT);
        }
      }
    } catch (ParseException e) {
      loginfoOperateService.newTransaction4SaveLog(InetAddressTool.getLocalIpv4(),
          "DangdangItemUpdateServiceImpl", LogInfoBizTypeEnum.ECI_REQUEST.getValueString(),
    ...
    logger.info("定时任务结束--DangdangItemUpdateServiceImpl--获取当当商品数据");
    return false;
  }
}
