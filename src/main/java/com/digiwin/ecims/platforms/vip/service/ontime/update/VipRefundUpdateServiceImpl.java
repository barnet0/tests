...
@Service("vipRefundUpdateServiceImpl")
public class VipRefundUpdateServiceImpl extends UpdateTask implements OnTimeTaskBusiJob {

  private static final Logger logger = LoggerFactory.getLogger(VipRefundUpdateServiceImpl.class);
  ...
  public boolean timeOutExecute(String... args) throws Exception {
    logger.info("定时任务开始--进入-VipRefundUpdateServiceImpl--取得唯品会退供单数据---方法");
    ...
    scheduleTypeBuffer.append(VipCommonTool.REFUND_UPDATE_SCHEDULE_NAME_PREFIX);
    int prefixLength = VipCommonTool.REFUND_UPDATE_SCHEDULE_NAME_PREFIX.length();
    ...
    try {
      ...
      if (aomsShopList != null && aomsShopList.size() != 0) {
        for (int i = 0; i < aomsShopList.size(); i++) {
          ...
          taskScheduleConfig.setEndDateByMonth(sysDate, 6);
          vipApiService.syncOrdersData(taskScheduleConfig, aomsshopT, "");
      ...
      loginfoOperateService.newTransaction4SaveLog(InetAddressTool.getLocalIpv4(),
          "VipRefundUpdateServiceImpl", LogInfoBizTypeEnum.ECI_REQUEST.getValueString(),
    ...
    logger.info("定时任务结束-VipRefundUpdateServiceImpl--取得唯品会退供单数据---方法");
    return false;
  }
