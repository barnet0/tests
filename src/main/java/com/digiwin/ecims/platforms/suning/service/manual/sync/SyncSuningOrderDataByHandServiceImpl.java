package com.digiwin.ecims.platforms.suning.service.manual.sync;
 ...
 *
 */
@Service("syncSuningOrderDataByHandServiceImpl")
public class SyncSuningOrderDataByHandServiceImpl implements SyncOrderDataByHandService {

  private static final Logger logger =
      LoggerFactory.getLogger(SyncSuningOrderDataByHandServiceImpl.class);

  private static final String CLASS_NAME = "SyncSuningOrderDataByHandServiceImpl";
  ...
  public String syncOrderDataByCreateDate(String storeId, String startDate, String endDate) {
    ...
    long totalSize = 0l;
    try {
      totalSize = suningApiSyncOrdersService.syncOrdersByCreated(startDate, endDate, storeId, null);
    } catch (IOException e) {
  ...
  public String syncOrderDataByModifyDate(String storeId, String startDate, String endDate) {
    ...
    long totalSize = 0l;
    try {
      totalSize = suningApiSyncOrdersService.syncOrdersByIncremental(startDate, endDate, storeId, null);
    } catch (IOException e) {
      loginfoOperateService.newTransaction4SaveLog(InetAddressTool.getLocalIpv4(),
          CLASS_NAME + "#" + SYNC_BY_CREATE_DATE, LogInfoBizTypeEnum.MANUAL_SYNC.getValueString(),
          new Date(), "IOException", e.getMessage(), "", "");
  ...
  public String syncOrderDataByOrderId(String storeId, String orderId) {
    SyncResOrderHandBean syncResOrderHandBean = null;
    try {
      suningApiSyncOrdersService.syncOrderByOrderId(storeId, orderId);
    } catch (IOException e) {
      loginfoOperateService.newTransaction4SaveLog(InetAddressTool.getLocalIpv4(),
